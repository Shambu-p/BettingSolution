...
function CreateCompany() {
    ...
    const prepareForm = async () => {
        ...
        setTimeout(() => {setShowWaiting(true);}, 10);
        let spec = getFormSpec(params.name ?? "");
        // if (!spec.roles.includes(loggedUser.Roles[0])) {
        //     setFieldValues({});
        //     return;
        // }
        let new_fields = await onFormLoadFunction[spec.onload](cookies.login_token, spec.fields, localData, loggedUser, params.r_id, spec.id);
        ...
        if(Utils.roleCheck(loggedUser.Roles, spec.canReadAttachment)) {
            loadAttachment();
        }
        ...
        setTimeout(() => {setShowWaiting(false);}, 10);
    }
    ...
    const actionCall = async (formAction: any, frm: any) => {
        ...
        try {
            let response: any = await buttonAction[formAction.action](
                ...
                frm.fields,
                loggedUser,
                (navData: any) => {navigate(navData.routeAddress)},
                (form?.id ?? "")
            );
            setAlert(response.message, "success");
                    ...
                    <div className="w-100 zpanel h-100" style={{overflow: "hidden", display: "flex", flexDirection: "column"}}>
                        {(form) ? (
                            <div className="d-flex justify-content-between align-itmes-center pt-2 pb-2 ps-4 pe-4" style={{backgroundColor: "rgba(125, 125, 125, 0.074)"}} >
                                ...
                                <div className="btn-groups">
                                    ...
                                    {
                                        form.actions.map((action: any) => (((params.r_id == "-1" && action.showOnNewForm) || (params.r_id != "-1")) && Utils.roleCheck(loggedUser.Roles, action.roles) && ((!action.condition) || (action.condition && (actionCondition[action.condition](cookies.login_token, form.fields, loggedUser))))) ? (
                        ...
                        {form ? (
                            <div className="row p-0 m-0 h-100" style={{position: "relative"}}>
                                ...
                                <div className={(params.r_id != "-1" && Utils.roleCheck(loggedUser.Roles, form.canReadAttachment)) ? "col-sm-12 col-md-12 col-lg-9 h-100 pb-3" : "col-12 h-100 pb-3"} style={{overflowX: "hidden", overflowY: "auto"}}>
                                    <div className="container py-3">
                                        <div className="form_section w-100 mb-4">
                                            <div className="row m-0 p-0">
                                                {
                                                    form?.fields.map((field: any, field_index: number) => {
                                                        if (!field.visible) {
                                                            ...
                                                            if ([FieldTypes.TEXT, FieldTypes.EMAIL, FieldTypes.PASSWORD, FieldTypes.SELECTCOLOR].includes(field.type)) {
                                                                return (
                                                            ...
                                                            if ([FieldTypes.DATE, FieldTypes.DATETIME].includes(field.type)) {
                                                                return (
                                                                    <div key={field.id} className="col-sm-12 col-md-6 col-lg-4">
                                                                        <div className="mb-3">
                                                                            <label className="form-label small_text">
                                                            ...
                                                            if ([FieldTypes.TEXTAREA, FieldTypes.JSON].includes(field.type)) {
                                                                return (
                                                                    <div key={field.id} className="col-12">
                                                                        <div className="mb-3">
                                                                            <label className="form-label small_text" id={`input_label_${field.id}`}>
                                            ...
                                            {
                                                form.actions.map((action: any) => (((params.r_id == "-1" && action.showOnNewForm) || (params.r_id != "-1")) && !action.notBelow && Utils.roleCheck(loggedUser.Roles, action.roles) && ((!action.condition) || (action.condition && (actionCondition[action.condition](cookies.login_token, form.fields, loggedUser))))) ? (
                                                    <button className={`btn btn-sm me-3 small_text ${action.class}`} onClick={() => { actionCall(action, form) }}>{action.lable}</button>
                                                ) : (<></>))
                                            }
                                        </div>
                                        ...
                                        {
                                            (Utils.roleCheck(loggedUser.Roles, form.activityRoles) && params.r_id != "-1") && (
                                                <RecordActivity recordId={params?.r_id} tableName={params?.name} />
                                                ...
                                                //         <button className="btn w-100 d-flex justify-content-between align-items-center zbtn accordion-header">
                                                //             <span>Activity History</span>
                                                //              <KeyboardDoubleArrowDownIcon className="mb-0" sx={{fontSize: "15px"}} />
                                                //         </button>
                                                //     </h5>

                                                //     <div id="flush-collapseOne" className="w-100 accordion-collapse expanded" data-bs-parent="#accordionFlushExample">
                                                //         {
                                        ...
                                        {(allRelatedList.length > 0 && params.r_id != "-1") ? (
                                            <div className="w-100">
                                                <ul className="nav nav-tabs">
                                                    {allRelatedList.filter((rtbl: any) => (Utils.roleCheck(loggedUser.Roles, rtbl.readRoles))).map(rl => (<li key={`rl_selector_${rl.id}`} className="nav-item p-0 zpanel">
                                                        <button
                                                ...
                                                {allRelatedList.filter((rtbl: any) => (Utils.roleCheck(loggedUser.Roles, rtbl.readRoles))).map(rl => (<div
                                                    key={`rl_body_${rl.id}`}
                                                    className="w-100 p-2"
                                ...
                                {
                                    ( Utils.roleCheck(loggedUser.Roles, form.canReadAttachment) && params.r_id != "-1" && !isMobile) && (
                                        <div className="col-sm-12 col-md-12 col-lg-3 p-0 border-start h-100" style={{display: "flex", flexDirection: "column", background: "var(--main_bg)", overflowY: "auto", overflowX: "hidden", position: "relative"}}>
                                            ...
                                            {
                                                (Utils.roleCheck(loggedUser.Roles, form.canAddAttachment)) ? (
