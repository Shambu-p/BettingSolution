...
import CustomeSelectBox from "./Reusables/CustomeSelectBox";
...
function FilterWindow({ form, conditions, filter, closeWindow }: {
    ...
    const loadContent = async () => {
        let allFields: any[] = await onFormLoadFunction[form.onload](cookies.login_token, form.fields, localData, loggedUser, -1);
        console.log("initial condition", conditions);
        setFields(allFields.filter(af => (Utils.roleCheck(loggedUser.Roles, af.readRoles))).map(fld => {
    ...
    const valueMapper = (new_value: {operator: string, value: any, type: string}) => {
        ...
        if(new_value.type == FieldTypes.NUMBER) {
        ...
        let temp_fields = fields.map(fld => {
            ...
            if(temp[fld.id]) {
                temp[fld.id] = valueMapper({
                    ...
                    value: fld.type == FieldTypes.BOOLEAN ? (fld.value == "true" ? true : false) : fld.value,
                    type: fld.type
                });
            }
    ...
    const setColumn = (event: any) => {
        ...
        if(selectedColumn != "" && !conditionFilter[selectedColumn]) {
            ...
            let found_field = fields.find(vl => (vl.id == selectedColumn && ![FieldTypes.IMAGE].includes(vl.type)));
            if(found_field){
            ...
            <div className="card-body" style={{ height: "100%", overflow: "auto" }}>
                <div className="mb-3 d-flex justify-content-start">
                    ...
                    <select
                        ...
                        className="form-control me-3 zinput form-control-sm"
                    ...
                    <button disabled={selectedColumn == ""} className="btn btn-sm zbtn" style={{ width: "max-content", fontSize: "13px" }} onClick={setColumn}>
                        <AddIcon sx={{ fontSize: 15, marginRight: "5px" }} />
                        Add Condition
                    </button>
                </div>
                <hr className="mb-4" />
                {
                    fields.map(field => {
                        if (field.notFilter || !conditionFilter[field.id]) {
                            ...
                            if ([FieldTypes.TEXT, FieldTypes.TEXTAREA, FieldTypes.EMAIL, FieldTypes.NUMBER, FieldTypes.FLOAT, FieldTypes.DATE, FieldTypes.DATETIME].includes(field.type)) {
                                return (
                                    <div key={field.id} className="mb-3 d-flex justify-content-between">
                                        <label className="form-label my-0" style={{width: "35%", fontSize: "13px"}}>
                                            <button className="btn btn-sm btn-danger me-3" style={{ width: "max-content", fontSize: "13px" }} onClick={() => {removeColumnCondition(field.id);}}>
                                        ...
                                        >
                                            ...
                                            {([FieldTypes.NUMBER, FieldTypes.FLOAT].includes(field.type)) && (
                                            ...
                                            {([FieldTypes.DATE, FieldTypes.DATETIME].includes(field.type)) && (
                                                <option value={Operators.LESS} selected={(Operators.LESS == field.filter)}>Before</option>
                                            )}
                                            {([FieldTypes.NUMBER, FieldTypes.FLOAT].includes(field.type)) && (
                                                <option value={Operators.GREATER} selected={(Operators.GREATER == field.filter)}>Greater Than</option>
                                            )}
                                            {([FieldTypes.DATE, FieldTypes.DATETIME].includes(field.type)) && (
                                                <option value={Operators.GREATER} selected={(Operators.GREATER == field.filter)}>After</option>
                                            )}
                            ...
                            if (FieldTypes.SELECT == field.type) {
                                return (
                                    <div key={field.id} className="mb-3 d-flex justify-content-between">
                                        <label className="form-label d-flex my-0" style={{width: "35%", fontSize: "13px"}}>
                                            <button className="btn btn-sm btn-danger me-3 py-1" style={{ width: "max-content", fontSize: "13px" }} onClick={() => {removeColumnCondition(field.id);}}>
                                                <CloseIcon sx={{ fontSize: 15 }} />
                                            </button>
                                            {field.label}
                                        </label>
                                    ...
                                    <div key={field.id} className="mb-3 d-flex justify-content-between">
                                        ...
                                        <select
                                            ...
                                            onChange={async (event: any) => {
                                                ...
                                                // tmp.value = event.target.value == "true";
                                                fieldSetter(tmp);
                                            }}
                                        >
                                            <option value="">None</option>
