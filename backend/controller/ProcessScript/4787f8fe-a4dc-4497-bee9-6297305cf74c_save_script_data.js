...
module.exports = async function(process_id, actions, optionalData) {
    ...
    let iterator = actions.getProcessVariable("iterate_post_items");

    if(iterator.current) {
        actions.setProcessVariable(iterator.current, iterator.current_index);
    }

    return iterator.current_index || 0;
