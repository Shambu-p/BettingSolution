...
module.exports = (dependencies) => {
    ...
    router.post("/password_reset", AuthService.authenticate, async (req, res, next) => {
        ...
        try{
            ...
            if(!Utils.roleCheck(req.user.Roles, [UserRoles.Admin])) {
            ...
            const userUpdate = await dependencies.databasePrisma.user.update({
                where: {
                    sys_id: userFind.sys_id,
                },
                data: userFind
            });

            await smsService.sendSMS(`0${new_phone}`, `Dear ${userFind.full_name}, your account password has been reset to ${password}`);
